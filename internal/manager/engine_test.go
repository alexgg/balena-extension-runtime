...
func TestParseStatusCode(t *testing.T) {
	tests := []struct {
		...
		{name: "200 OK", line: "HTTP/1.1 200 OK\r\n", want: 200},
		{name: "404 Not Found", line: "HTTP/1.1 404 Not Found\r\n", want: 404},
		{name: "201 Created", line: "HTTP/1.1 201 Created\r\n", want: 201},
		{name: "204 No Content", line: "HTTP/1.1 204 No Content\r\n", want: 204},
		{name: "empty string", line: "", wantErr: true},
		{name: "no status code", line: "HTTP/1.1", wantErr: true},
		{name: "non-numeric code", line: "HTTP/1.1 abc OK\r\n", wantErr: true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			code, err := parseStatusCode(tt.line)
			if tt.wantErr {
				require.Error(t, err)
			} else {
